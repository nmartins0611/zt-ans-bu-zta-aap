...
NOTE: To pop out the instructions in a new browser tab, you can click on the lab title at the top.

image::instructions.png[link=self, window=blank, width=100%]
...
image::aaptfmain.png[link=self, window=blank, width=100%]
...
image::module1scenario.png[link=self, window=blank, width=100%]
...
image::labtopmenu.png[link=self, window=blank, width=100%]
...
* Select the user icon in the top left.
...
image::tfetoken0.png[link=self, window=blank, width=100%]

image::tfetoken.png[link=self, window=blank, width=100%]

image::tfetoken1.png[link=self, window=blank, width=100%]
...
image::tfeworkspace.png[link=self, window=blank, width=100%]
...
image::tfeworkspace1.png[link=self, window=blank, width=100%]

The workspace is now created. You can now use this workspace for Terraform Enterprise runs.

image::tfeworkspace2.png[link=self, window=blank, width=100%]
...
image::tfevariables.png[link=self, window=blank, width=100%]
...
image::tfevariables1.png[link=self, window=blank, width=100%]
...
Expand: Automation Execution -> Infrastructure -> Credential Types -> + Create Credential Type
...
image::tfecredtype.png[link=self, window=blank, width=100%]
...
| https://tfe-https-{guid}.{domain}    **<Your terraform hostname url>** #This will look like: https://tfe-https-12345-a.apps.#######.rhdp.net
...
image::tfecred.png[link=self, window=blank, width=100%]
...
image::tfinventorysource0.png[link=self, window=blank, width=100%]
image::tfinventorysource.png[link=self, window=blank, width=100%]
...
NOTE: This could take up to a minute as this is the first time we use the Execution Environment, and it needs to pull down the image from the registry.
...
`Automation Execution` -> `Templates` -> `+ Create job template`, then click on `Create job template`
...
image::templateapply1.png[link=self, window=blank, width=100%]

=== Create the `WF-Terraform Enterprise APPLY` workflow template, and it's nodes

* Click on the `Ansible Automation Platform` tab at the top of lab.
(if not already) Expand the `Automation Execution` menu on the left.
`Automation Execution` -> `Templates` -> `+ Create template` -> then select `Create workflow job template`
...
image::wftemplateapply1.png[link=self, window=blank, width=100%]
...
image::wftemplateapply2.png[link=self, window=blank, width=100%]

* Click on the vertical ellipsis `...` at the end of the node we just created, and then select `+ Add step and link`.
* For this next step change the `Node Type` to `Inventory Source Sync` and select the `AWS source for TFE resources`. Select `Next` then `Finish` to save this second step. This will synchronize the inventory source you created earlier.

image::wftemplateapply3.png[link=self, window=blank, width=100%]
...
image::wftemplateapply4.png[link=self, window=blank, width=100%]
...
image::wftemplateapply5.png[link=self, window=blank, width=100%]
...
This showing that Ansible Automation Platform was able to trigger the Terraform Enterprise `plan` and `apply` operations successfully.

image::tfeapply1.png[link=self, window=blank, width=100%]

=== Validate the Nginx installation

**Please make sure that the Ansible Workflow completed successfully before proceeding. AAP has to have completed the `Install Nginx on RHEL` job template successfully before proceeding to validate the Nginx installation.**
...
image::tfeapplyip.png[link=self, window=blank, width=100%]
...
* If prompted that the site doesn't support a secuure connection, please click on the `Continue to site` button. 
...
image::tfeapplyinventory.png[link=self, window=blank, width=100%]
...
* Expand the `Automation Execution` menu on the left. `Automation Execution` -> `Templates`.
...
* Click on the vertical ellipsis `...` at the end of the node we just created, and then select `+ Add step and link`.
* For this next step change the `Node Type` to `Inventory Source Sync` and select the `AWS source for TFE resources`. Select `Next` then `Finish` to save this second step.
...
image::tfedestroy1.png[link=self, window=blank, width=100%]
