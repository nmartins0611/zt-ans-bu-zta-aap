...
You have now created an Application Role (AppRole) in Vault Enterprise. An Ansible Automation Platform and Terraform Enterprise can use this to authenticate against the Vault secret engine.
...
Once you have saved the credential, go back into it, and select `Test` to validate this. We know we created the AWS credentials in the Vault secret engine, so this test should pass.
...
*This will be a machine credential to SSH to the inventory RHEL hosts, and it will use the Vault lookup credential we created earlier.*
