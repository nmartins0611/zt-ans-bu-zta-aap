...
* Select `+ Add step` and then change the `Node Type` to `Inventory Source Sync` and select the `AWS source for TFE resources`, Select `Next` to continue, and then `Finish` to save the node.
...
Now we're **ALMOST SET** to trigger the infrastructure provisioning and automation from Terraform Enterprise, in the next lab section.
