...
* For the `Which events would you like to trigger this webhook?` section, select `Just the push event.`
...
* Terraform Enterprise will then trigger the `WF-Launched by TFE` workflow template in Ansible Automation Platform.
...
* The RHEL ec2 instances will have Ngninx installed and running on them.
