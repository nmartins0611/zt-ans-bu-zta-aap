...
NOTE: The Terraform binary is only needed if you want to also leverage the **cloud.terraform** collection modules. 
...
. The `cloud.terraform` collection is for community Terraform. While it can be made to work with Terraform Enterprise (and HCP Terraform), but that is what the `hashicorp.terraform` collection is for.
...
NOTE: You will find a copy of the `execution-environment.yml` and `requirements.yml` files in the repository you imported in an earlier module.
