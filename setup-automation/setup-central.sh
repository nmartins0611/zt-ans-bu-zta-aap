...
###############################################################################
# Helpers (RESTORED - Required for later steps)
###############################################################################
...
###############################################################################
# Ansible Collections & IPA Config
###############################################################################
...
podman rm --force keycloak || true

# Note: Ensure GUID and DOMAIN variables are set
podman create --name keycloak --restart=always -p 8180:8080 -p 8543:8443 \
  ...
  -e KC_HOSTNAME=keycloak-https-${GUID:-default}.${DOMAIN:-local} \
  ...
  --hostname=keycloak-https-${GUID:-default}.${DOMAIN:-local} \
...
ansible-playbook -i "$INVENTORY" "$PLAYBOOK_DIR/configure-dns.yml"
ansible-playbook -i "$INVENTORY" "$PLAYBOOK_DIR/configure-vault.yml"
ansible-playbook -i "$INVENTORY" "$PLAYBOOK_DIR/configure-vault-ssh.yml"
ansible-playbook -i "$INVENTORY" "$PLAYBOOK_DIR/enroll-idm-clients.yml"
